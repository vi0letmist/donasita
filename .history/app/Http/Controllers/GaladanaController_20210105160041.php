...
class GaladanaController extends Controller
{
    ...
    public function tost()
    {
        ...
        $galadana = galadana::join('users', 'users.id','=', 'galadana.user_id')
            ...
            ->paginate(9);
        if(request()->ajax()) {
            return DataTables::of(Galadana::join('users', 'users.id','=', 'galadana.user_id')
            ->select(DB::raw('@rownum  := @rownum  + 1 AS rownum'),'users.*', 'galadana.*')
            ->orderBy('galadana.created_at','desc')->select([
                DB::raw('@rownum  := @rownum  + 1 AS rownum'),
                'judul' , 'slug', 'created_at'
            ]))
            ->addIndexColumn()
