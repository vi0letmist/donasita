...
class GaladanaController extends Controller
{
    ...
    public function post($slug)
    {
        ...
        $sideDonate = Galadana::join('donates', 'donates.galadana_id', '=', 'galadana.id')
                ...
                ->take(3)
                ->get();
        $twitter = Share::page('http://localhost:8000/g/'.$galadana->slug, $galadana->title)
                ->twitter()
                ->getRawLinks();
        $facebook = Share::page('http://localhost:8000/g/'.$galadana->slug, $galadana->title)
                    ->facebook()
                    ->getRawLinks();
        $reddit = Share::page('http://localhost:8000/g/'.$galadana->slug, $galadana->title)
                    ->reddit()
                    ->getRawLinks();
        $telegram = Share::page('http://localhost:8000/g/'.$galadana->slug, $galadana->title)
                    ->telegram()
                    ->getRawLinks();
        $whatsapp = Share::page('http://localhost:8000/g/'.$galadana->slug, $galadana->title)
                    ->whatsapp()
                    ->getRawLinks();
