    ...
    public function tost()
    {
        ...
        $galadana = galadana::join('users', 'users.id','=', 'galadana.user_id')
            ->select([
                ...
                'galadana.judul' , 'galadana.cerita', 'galadana.created_at', 'galadana.status'
            ]);
        if(request()->ajax()) {
            return DataTables::of($galadana)
            ->addIndexColumn()
            ->addColumn('cerita', function($galadana){
                $cerita = htmlspecialchars_decode(\Illuminate\Support\Str::limit($galadana->cerita, $limit = 40, $end = "..."));
                return $cerita;
            })
            ...
            ->rawColumns(['column_with_html']),['status'],['action'])
            ->make(true);
        }
