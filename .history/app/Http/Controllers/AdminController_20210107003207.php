    ...
    public function managepost()
    {
        ...
        $galadana = galadana::join('users', 'users.id','=', 'galadana.user_id')
            ...
            ->select([
                ...
                'galadana.id','galadana.judul' , 'galadana.cerita','users.name', 'galadana.created_at', 'galadana.status', 'galadana.slug'
            ]);
        if(request()->ajax()) {
            ...
            ->addColumn('action', function($galadana){
                ...
                $btn = $btn.'<a data-id="'.$galadana->id.'">
                <button class="mr-2 btn-icon btn-icon-only btn btn-sm btn-outline-danger deleteGaladana"><i class="pe-7s-trash btn-icon-wrapper"> </i></button></a>';
                return $btn;
            })
