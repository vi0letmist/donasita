...
class GaladanaController extends Controller
{
    ...
    public function post($slug)
    {
        ...
        $sideDonate = Galadana::join('donates', 'donates.galadana_id', '=', 'galadana.id')
                ...
                ->take(3)
                ->get();
        $share = ShareFacade::page('http://localhost:8000/g/'.$galadana->slug, $galadana->title)
                ->facebook()
                ->twitter()
                ->reddit()
                ->telegram()
        ...
        $whatsapp = ShareFacade::page('http://localhost:8000/g/'.$galadana->slug, $galadana->title)
                    ->whatsapp()
                    ->getRawLinks();
        return view('campaign.post', compact('galadana','author','donate', 'sideDonate','share', 'twitter', 'facebook','reddit','telegram','whatsapp'));
    }
